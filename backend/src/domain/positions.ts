 ...
 * 六位状态（L1–L6）单一语义定义。
 *
 * ⚠️ 这是后端权威定义。App 端（src/domain/positions.ts）保留一份逐字一致的副本，
 * 并在状态 API（`/api/data/state`）的 `positions` 字段中回传本定义，
 * 保证「前后端 / 数据库 / 提示词 / 界面」只有一套语义，且同一位置在各处含义一致。
 *
 * 每个位置都有明确的「内观来源（主观）」与「外观来源（客观）」，二者不得混用同一条记录。
 */
...
export interface PositionDef {
  ...
  /** 展示标签 */
  label: string;
  /** 内观（inner）仅来自：主观体验、价值、解释 */
  innerSource: string;
  /** 外观（outer）仅来自：日历、任务、作品、健康、现实反馈 */
  outerSource: string;
  /** 简短说明（克制表述，不宣称能"理解/预测/模拟"用户） */
  desc: string;
}
