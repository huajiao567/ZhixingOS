...
import { isNativeLocalOnlyRuntime } from '../../services/runtimePolicy';

export function AuthScreen() {
  const theme = useAppTheme();
  const ts = useTextStyles();
  const { login, register } = useAuth();
  const nativeLocal = isNativeLocalOnlyRuntime();
  ...
  useEffect(() => {
    if (!nativeLocal) getApiBase().then(setServerUrl).catch(() => undefined);
  }, [nativeLocal]);
  ...
  return (
    ...
    >
      <View style={[styles.brand, { marginBottom: theme.spacing.xl + theme.spacing.xs }]}>
        ...
        {nativeLocal ? (
          <Text style={[ts.tertiary, { color: theme.colors.green, marginTop: theme.spacing.sm, lineHeight: 18 }]}>原生本地模式 · 个人数据和算法不依赖后端服务器</Text>
        ) : null}
      </View>
      ...
      <Text style={[ts.tertiary, styles.foot, { marginTop: theme.spacing.xl }]}>
        {nativeLocal
          ? '记录、数字孪生、照片/语音文件与确定性算法默认只留在此设备。只有你主动启用的大模型 API 会发送本次明确提供的文本。'
...
const styles = StyleSheet.create({
  ...
  input: { paddingVertical: 14, borderWidth: 1 },
