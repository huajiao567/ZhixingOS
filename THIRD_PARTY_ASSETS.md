...
- Web delivery: Expo serves this reviewed file once at `/avatar/AvatarSample_G.glb`. The Web renderer must not also static-`require` the same public asset, because that creates a second Metro-hashed ~15.3 MB copy in the production export. CI runs `npm run check:web-avatar-bundle` after `expo export --platform web` and rejects a missing, byte-different, or duplicated production VRM. Native remains separate: `AvatarModelView.native.tsx` keeps a static require so release APK/IPA bundling can include the reviewed model.
